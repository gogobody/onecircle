...
class contents{
    ...
    public static function biliVideo($text)
    {
        ...
        if (preg_match($reg, $text)) {
            $replacement = '<div class="embed-responsive embed-responsive-4by3"><iframe class="video embed-responsive-item" src="//player.bilibili.com/player.html?bvid=$1&page=$2" scrolling="no" border="0" frameborder="no" framespacing="0" allowfullscreen="true"> </iframe></div>';
    ...
    public static function video($text)
    {
        $reg = '/\[video src="(.+?)"]/sm';
        if (preg_match($reg, $text)) {
            $replacement = '<div class="embed-responsive embed-responsive-4by3"><iframe class="video" src="$1" scrolling="no" border="0" frameborder="no" framespacing="0" allowfullscreen="true"></iframe></div>';
            return preg_replace($reg, $replacement, $text);
        }
        return $text;
    }
